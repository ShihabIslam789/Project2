...
Roster::Roster(std::string input_file_name):ArrayBag<Student>(){	
	...
	&&getline(infile, first, ',') && getline(infile, last)){ //delimiter used
		id_int = std::convert(id); //  converts from string id to integer id 
		Student listing = Student(id_int, first, last); //creates student object from constructor
		add(listing); 
	}
}
